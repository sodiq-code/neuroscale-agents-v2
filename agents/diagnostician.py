...
Model: Gemini 2.0 Flash (Google GenAI SDK — root cause reasoning)
"""
...
try:
    ...
    _gemini_client = _genai.Client(api_key=os.environ.get("GOOGLE_API_KEY", ""))
...
class DiagnosticianAgent:
    ...
    MODEL = "gemini-2.0-flash"  # Root cause reasoning via Google GenAI SDK
    ...
    def _determine_root_cause(self, incident: dict, runbooks: list[RunbookResult]) -> dict:
        ...
        if GEMINI_AVAILABLE and _gemini_client:
            return self._gemini_root_cause(incident, runbooks)
    ...
    def _gemini_root_cause(self, incident: dict, runbooks: list[RunbookResult]) -> dict:
        ...
        ) if runbooks else "No historical runbooks matched."

        prompt = f"""You are an expert Kubernetes SRE agent diagnosing a machine learning inference service incident.

INCIDENT:
- Service: {incident['model_name']}
- Severity: {incident['severity']}
- P99 Latency: {metrics.get('p99_latency_ms', 0):.0f}ms (SLO threshold: {config.LATENCY_P99_THRESHOLD_MS:.0f}ms)
- Error Rate: {metrics.get('error_rate_pct', 0):.1f}% (threshold: {config.ERROR_RATE_THRESHOLD_PCT:.1f}%)
- Total Spans: {metrics.get('total_spans', 0)}
- Agent Hypothesis: {incident.get('agent_hypothesis', 'N/A')}
...
{{
  "type": "CPU_THROTTLING" | "MODEL_DRIFT" | "RESOURCE_EXHAUSTION" | "ARGOCD_SYNC_FAILURE" | "KYVERNO_POLICY_DENIAL",
  "confidence": "HIGH" | "MEDIUM" | "LOW",
  "description": "<one sentence root cause description>",
  "fix_type": "resource_limit_increase" | "model_rollback" | "argocd_sync" | "policy_fix",
  "reasoning": "<2-3 sentences explaining your diagnosis>"
}}"""
        ...
        try:
            ...
            return {
                ...
                "description": result.get("description", "Gemini-diagnosed incident"),
                "affected_resource": f"apps/{incident['model_name']}/inference-service.yaml",
                "fix_type": result.get("fix_type", "resource_limit_increase"),
                "runbook_ref": runbooks[0].file if runbooks else "RB-001-cpu-throttling-kserve.md",
                "gemini_reasoning": result.get("reasoning", ""),
            }
        except Exception as e:
            print(f"  ⚠️  Gemini call failed ({e}), falling back to rule-based classification")
            return self._rule_based_root_cause(incident, runbooks)

    def _rule_based_root_cause(self, incident: dict, runbooks: list[RunbookResult]) -> dict:
        """Fallback rule-based root cause classification (used in DEMO_MODE)."""
